...
if nargin < 3, trial_level_fname='mmrescan_trial_outputs_by_timestep.csv'; end
...
%changed x.muPhi to x.muPhi' so that horzcar works
...
model_info = cellfun(@(x) {x.sceptic_settings.id, x.sceptic_settings.dataset, x.sceptic_settings.model, ...
  char(x.sceptic_settings.evo_fname), char(x.sceptic_settings.obs_fname)}, ...
...
for i = 1:ns
  ...
  if any(strcmpi(this_subj.sceptic_settings.model,{'policy', 'countertrustee','utilitygain', 'null', 'regret', 'ptrustee', 'prs','discrepancy','mixed','rcounter','rcounter2','rcounter3','mixed2'}))
  ...
  t_tbl = table(repmat(string(this_subj.id), n_trials, 1), repmat(this_subj.sceptic_settings.dataset, n_trials, 1), ...
