...
if strcmp(os(1:end-2),'PCWIN')
    ...
    if strcmp(me,'polinavanyukov')==1
        ...
        %models_location = glob('/Users/polinavanyukov/Scripts/clinical_scanSEP04/f_trust_Qlearn_policy/');
        pst_regs_location= glob('/Users/polinavanyukov/Regs/clinical_basic_regs/');
        write_location = strcat('/Users/polinavanyukov/Regs/clinical_policy_PEsValues/',date);
        %write_location = strcat('/Users/polinavanyukov/Regs/clinical_policy_PEsValues_FixedPs/',date);
    else
        pe_location = glob('?');
        values_location = glob('?');
...
%for index = 1:num_of_subjects    
    ...
    for block =1:blocks
        ...
        pes(block_beg:block_end) = posterior.muX(block*2,block_beg:block_end);
    end
    ...
    %% Alignment check
         ...
         subplot(4,1,1);
         hold on;
         plot(b.decisions(1:10),'blue');
         subplot(4,1,2);
         plot(trustee.decisions(1:10),'red');
         subplot(4,1,3);
         plot(pes(1:10), 'red');
         subplot(4,1,4);
         plot(values(1:10));
         % PEs and values need to be shifted one over to the left, i.e.,
         % PE and value  @ trial = 2 should start @ trial = 1.
         
        %% Shifting PEs and values
        ...
        %% Flipping PEs and values (or multiplying by the decision vector
        values(kept) = values(kept)*-1; %flipping values  
        pes(kept) = pes(kept)*-1;
        
        %% Z-scoring
        not_missing = ~noresponse;
        not_missing(192) = 0;
        stdev = std(values(not_missing));
        values(not_missing) = values(not_missing) - mean(values(not_missing)); %z-scoring
        values(not_missing) = values(not_missing)./stdev;
        
        stdev = std(pes(not_missing));
        pes(not_missing) = pes(not_missing) - mean(pes(not_missing));
        pes(not_missing) = pes(not_missing)./stdev;
        ...
        dlmwrite([write_location '/trust' num2str(id) 'pes_policy_FT.dat'],pes_FT,'delimiter','\t','precision','%.6f');
        
        %dlmwrite([write_location '/trust' num2str(id) 'values_policy_fixedPs_FT.dat'],values_FT,'delimiter','\t','precision','%.6f');
        %dlmwrite([write_location '/trust' num2str(id) 'pes_policy_fixedPs_FT.dat'],pes_FT,'delimiter','\t','precision','%.6f');
