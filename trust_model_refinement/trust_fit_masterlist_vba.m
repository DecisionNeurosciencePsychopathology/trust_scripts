...
counter = 7;                    %counter = 0: no counterfactual feedback; counter = 1: regret; counter = 2: subject/policy; 3: trustee; 4: other type of regret (reviewers); else counter = 5: disappointment (reviewers); counter = 6: SVM model
                                %counter = 7: 2 actions tracked; reward on trial = p1 x actual
                                %rewards + p2 x policy rewards;
                                %counter = 8; 1 action tracked - Q(invest);
                                %reward on trial = p1 x policy + (1 - p1) x
                                %regret (for keep trials only)
                                %
...
for index=1:length(masterlist.trust_ids)
    ...
    if counter ~= 6 %not SVM model
        try
            [posterior, out] = trust_Qlearning(datalocation{:}, id, counter, multisession,...
