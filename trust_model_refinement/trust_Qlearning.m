function [posterior,out] = trust_Qlearning(datalocation,id, counter, multisession, fixed, sigmakappa, censor, save_str)
...
elseif counter ==7
    f_fname = @f_trust_Qlearn_actualNpolicy;            %reward on trial = p1 * actual + (1-p1) * policy
end
...
if counter == 7
    g_fname = @g_trust_softmax_ks_kt_2actions;
end
...
n_hidden_states = 3; %track the value of sharing, keeping and PE
...
if multisession
    ...
    elseif fixed == 3
        ...
        priors.SigmaX0 = diag([0 0]);   %infinite precision priors set on the initial value and PEs
    elseif fixed == 4
        %learning rate varies between sessions
        options.multisession.fixed.phi = 'all'; %fixing the beta and kappa parameters to be the same across sessiosn
        options.multisession.fixed.X0 = 'all';
        priors.SigmaX0 = diag([0 0]);   %infinite precision priors set on the initial value and PEs
    elseif fixed == 5
        %beta varies between sessions
...
if counter == 7
    n_theta = 2; %learning rate p, actual reward contribution p and policy-style reward contribution (1-p)
end
